...
{/* Reuse the client modal for contracts */}{/* Reuse the client modal for contracts */}
  ...
  if (activeTab === 'agenda_fixa') {
    ...
    const modalOverlay: React.CSSProperties = { position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', overflowY: 'auto', zIndex: 100000 };
    const modalBox: React.CSSProperties = { background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '16px', width: '90%', maxWidth: '500px', maxHeight: 'calc(100vh - 32px)', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.5)', margin: 'auto' };
            ...
            <div style={{ ...modalBox, maxWidth: '500px' }}>
              ...
              <form onSubmit={handleCreateFixedSchedule}>
                ...
                <div style={{ padding: '15px 24px', borderTop: '1px solid var(--border-color)', display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                  <button type="button" style={btnSecondary} disabled={isSavingFixedSched} onClick={() => setShowFixedSchedModal(false)}>Cancelar</button>
                  <button 
                    type="submit" 
